...
router.route('/subjects')
    ...
    .get((req, res) => {
        ...
        {
            myRes.push({subject:courses[i].subject, className:courses[i].className});
        }
        res.send(myRes);
    })

router.route('/subjects/:subject')
    ...
    .get((req, res) => {
        const subjects = courses.filter(s => s.subject === req.params.subject);
        if (subjects.length > 0){
            var myRes = [];
            for (var i = 0; i < subjects.length; i++)
            {
                myRes.push({catalog_nbr:subjects[i].catalog_nbr});
            }
            res.send(myRes);
        }
        else{
...
//get optional component of course of specific subject
router.route('/subjects/:subject/:courseCode/:opComponent?')
    // Get the timetable entry for a given subject code, a course code and an optional course component.
    .get((req, res) => {
        const subjects = courses.filter(s => s.subject === req.params.subject);
        if (subjects.length > 0){
            const course = subjects.filter(c => c.catalog_nbr == req.params.courseCode);
            if (course){
                if (req.params.opComponent)
                {
                    var component;
                    var exists = false;
                    for(var i = 0; i < course.length; i++)
                    {
                        if (course[i].course_info[0].ssr_component === req.params.opComponent)
                        {
                            component = course[i];
                            exists = true;
                        }
                    ...
                    if (exists)
                    {
                        ...
                        res.status(404).send(`Course ${req.params.subject} ${req.params.courseCode} does not have component ${req.params.opComponent}`);
                    }
                }
                else {
                    res.send(course);
                ...
                res.status(404).send(`Course ${req.params.courseCode} was not found within subject ${req.params.subject}!`);
            }
        }
        else{
            res.status(404).send(`Subject ${req.params.subject} was not found!`);
...
router.route('/schedules')
    ...
    .get((req, res) => {
        ...
        {
            myRes.push({name:schedules[i].name, numCourses:schedules[i].courses.length, id:schedules[i].id});
        }
        res.send(myRes);
    })
    ...
    .post((req, res) => {
        ...
        if (newSchedule.name) {
            ...
            if (exists){
                res.status(400).send(`Schedule name ${newSchedule.name} already exists`)
            }
            else {
...
router.route('/schedules/:name')
    ...
    .get((req, res) => {
        ...
        if (schedule)
        {
            schedule.numCourses = schedule.courses.length;
            res.send(schedule);
        }
        else{
            res.status(400).send(`Schedule with name \'${req.params.name}\' does not exist`);
        }
    })
    // update/replace existing schedule
    .put((req, res) => {
        ...
        //replace that schedule with a new one
        const schedule = schedules.findIndex(s => s.name === newSchedule.name);
        if (schedule < 0){ //schedule not found
            res.status(400).send(`Schedule with name ${req.params.name} does not exist`);
        }
        else{
            console.log('Modifying Schedule ', req.params.name);
    ...
    .delete((req, res) => {
        const index = schedules.findIndex(s => s.name === req.params.name);
        if (index < 0)
        {
            res.status(400).send(`Schedule with name \'${req.params.name}\'does not exist`);
        }
